...
import { EXPENSES, CATEGORIES } from '../data/dummyData';
...
export default function ExpensesPage() {
  ...
  const [form, setForm] = useState({
    merchant: '', amount: '', category: CATEGORIES[0].name,
    date: '', type: 'משתנה', payment: 'ויזה מזרחי', notes: ''
  });

  const cats = ['הכל', ...new Set(EXPENSES.map(e => e.category))];

  const filtered = expenses.filter(e => {
    const matchSearch = !search || e.merchant.includes(search) || e.category.includes(search);
  ...
  const handleAdd = () => {
    ...
    setForm({ merchant: '', amount: '', category: CATEGORIES[0].name, date: '', type: 'משתנה', payment: 'ויזה מזרחי', notes: '' });
    setShowForm(false);
  };
    ...
    <div className="expenses-page">
      ...
      <div className="exp-header">
        <div>
          ...
          <p className="exp-subtitle">{filtered.length} הוצאות · סה"כ {fmt(total)}</p>
        </div>
        ...
        <div className="card exp-form">
          ...
          <div className="exp-form-grid">
            <div className="field">
              ...
              <input className="input" placeholder="לדוגמה: שופרסל" value={form.merchant}
            ...
            <div className="field">
              ...
              <input className="input" placeholder="אופציונלי" value={form.notes}
      ...
      <div className="card exp-filters">
        <input className="input exp-search" placeholder="חיפוש לפי שם עסק..." value={search}
        ...
        <div className="exp-filter-row">
          ...
          <select className="select exp-type-select" value={typeFilter}
      ...
      <div className="exp-list">
        {filtered.length === 0 && (
          <div className="exp-empty">לא נמצאו הוצאות תואמות</div>
        )}
        {filtered.map(exp => {
            ...
            <div key={exp.id} className="expense-row">
              ...
              <div className="expense-info">
                ...
                <div className="expense-meta">
                  {exp.date} · <span className="badge badge-accent">{exp.category}</span>
                  {' '}· {exp.type} · {exp.payment}
                </div>
              </div>
