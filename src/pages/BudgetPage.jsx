...
import { CATEGORIES, EXPENSES } from '../data/dummyData';
  ...
  return (
    <div className="budget-page">
      ...
      <div className="budget-grid">
        {rows.map(row => (
          <div key={row.name} className={`card budget-card budget-card-${row.cls}`}>
            <div className="budget-card-top">
              <div className="budget-card-left">
                <span className="budget-icon">{row.icon}</span>
                <div>
                  <div className="budget-cat-name">{row.name}</div>
                  <div className="budget-cat-meta">יעד: {fmt(row.budget)}</div>
                </div>
              </div>
              <div className={`badge badge-${row.cls}`}>
                {row.pct}%
              </div>
            </div>

            <div className="progress-track" style={{ marginBottom: 8 }}>
              <div className={`progress-fill ${row.cls}`} style={{ width: `${row.pct}%` }} />
            </div>
            ...
            <div className="budget-card-stats">
              ...
              <div className="budget-stat">
                ...
                <strong className={row.left >= 0 ? 'amount-good' : 'amount-bad'}>
      ...
      <div className="card">
        ...
        <div className="nobud-list">
          {CATEGORIES.filter(c => c.budget == null).map(cat => {
            const spent = EXPENSES
              .filter(e => e.category === cat.name)
              .reduce((s, e) => s + e.amount, 0);
            return (
              <div key={cat.name} className="nobud-row">
                ...
                <span className="nobud-amount">{spent > 0 ? fmt(spent) : '—'}</span>
              </div>
            );
          })}
        </div>
